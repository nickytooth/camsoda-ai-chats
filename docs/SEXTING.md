...
6. С **един Gemini call** оценява съобщението: signals (charm/respect/humor/vulgarity/pushing) **и** дали е NSFW.
...
- `vulgarity` е негативно само за истински груб/обиден език (обиди, slurs) — **не** за нормален sexual talk
...
```python
if respect <= -3 or vulgarity <= -5:      return "annoyed", 3
if pushing <= -3 or ...:                  return "distant", 2
